...
burst_counts_by_node_enclosure = {}
for bur in node_bursts:
  node_count = len(bur['nodes'])
  enclosure_count = len(list(dict.fromkeys(bur['nodes'])))
  key = (enclosure_count, node_count)
  if key in burst_counts_by_node_enclosure:
    burst_counts_by_node_enclosure[key] += 1
  else:
    burst_counts_by_node_enclosure[key] = 1


print(burst_counts_by_node_enclosure)

with open('burst_node_enclosure.csv', 'w') as the_file:
  the_file.write('enclosures,drives,count\n')
  for burst in burst_counts_by_node_enclosure:
    count = burst_counts_by_node_enclosure[burst]
...
plt.legend()
plt.xlabel('Number of enclosure affected')
plt.xscale("log")
plt.ylabel('Number of drives affected')
plt.yscale("log")
plt.title('Frequency of failure bursts sorted by enclosures and drives affected')
...
plt.savefig('ornl_bursts_node_enclosure.png')