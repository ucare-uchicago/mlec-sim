...
class MLEC_D_C_RS3(Policy):
    ...
    def update_diskgroup_state(self, event_type, diskId):
        # self.simulation.log.append("    self.failed_spools_undetected:{}".format(self.failed_spools_undetected))
        if event_type == Disk.EVENT_FAIL:
            disk = self.disks[diskId]
            spool = self.spools[disk.spoolId]
            rackId = disk.rackId
        ...
        if event_type == Disk.EVENT_DETECT:
            ...
            if len(spool.failed_disks_in_repair) > self.sys.m:
                ...
                min_disk_remaining_data = self.sys.diskSize
                spool_repair_diskId = diskId
                for failedDiskId in spool.failed_disks_in_repair:
                    failed_disk = self.disks[failedDiskId]
                    if failed_disk.curr_repair_data_remaining <= min_disk_remaining_data:
                        spool_repair_diskId = failedDiskId
                        min_disk_remaining_data = failed_disk.curr_repair_data_remaining
                spool.failed_disks_network_repair[spool_repair_diskId] = 1
                spool.repair_data = min_disk_remaining_data
                spool.total_network_repair_data = min_disk_remaining_data
                return disk.spoolId
        ...
        if event_type == Spool.EVENT_REPAIR:
            ...
            assert (len(spool.failed_disks_network_repair) == 1), "spool.failed_disks_network_repair should only have one disk in repair!"
    ...
    def update_diskgroup_priority(self, event_type, spoolId, diskId):
        ...
        if event_type == Disk.EVENT_DETECT:
            ...
            # if spool.priority == 0:
            #     self.simulation.log.append("spoolid {}  priority {}".format(spoolId, spool.priority))
    ...
    def calc_spool_repair_time(self, spool, priority):
        ...
        # logging.info("spool {} priority {} total_repair_data_readwrite {} per_spool_total_repair_bandwidth {} repair_time {}".format(
        #     spool.spoolId, priority, total_repair_data_readwrite, per_spool_total_repair_bandwidth, repair_time
        # ))
    ...
    def manual_inject_failures(self, fail_report, simulate):
        ...
        # self.simulation.log.append("{}".format(pformat(fail_report)))
    ...
    def manual_inject_spool_failure(self):
        ...
        # self.simulation.log.append("{}".format(pformat(self.manual_spool_fail_sample)))
