...
from .pdl import mlec_d_sodp_pdl
from .repair import mlec_d_sodp_repair

class MLEC_D_SODP_RS1(Policy):
    #--------------------------------------
    ...
    def update_diskgroup_state(self, event_type, diskId):
        if event_type == Disk.EVENT_FAIL:
            ...
            if spool.disk_max_priority > self.sys.m:
                ...
                num_failed_disks = len(spool.failed_disks)
                # probability that a stripe is related to these failed disks
                prob_stripe_affected = 1.0 - float(math.comb(self.sys.spool_size-num_failed_disks, self.sys.n)) / math.comb(self.sys.spool_size, self.sys.n)
                # print(prob_stripe_affected)
                spool.lost_local_stripes = self.sys.diskSize * self.sys.spool_size * prob_stripe_affected
        ...
        if event_type == Spool.EVENT_REPAIR:
            ...
            detect_count = 0
            small_time_diff = 0.00000000000000000001
            for failedDiskId in sorted_undetected_disks:
                failed_disk = self.disks[failedDiskId]
                ...
                self.compute_disk_priority_percents(failed_disk)
                if failed_disk.failure_detection_time <= self.curr_time:
                    failed_disk.failure_detection_time = self.curr_time + small_time_diff * detect_count
                    heappush(self.simulation.failure_queue, (failed_disk.failure_detection_time, Disk.EVENT_DETECT, failedDiskId))
                else:
                    failed_disk.failure_detection_time = failed_disk.failure_detection_time + small_time_diff * detect_count
                    heappush(self.simulation.failure_queue, (failed_disk.failure_detection_time, Disk.EVENT_DETECT, failedDiskId))
                detect_count += 1
    ...
    def check_pdl(self):
        ...
        return mlec_d_sodp_pdl(self)
    
    def update_repair_events(self, event_type, diskId, repair_queue):
        mlec_d_sodp_repair(self, repair_queue)
        if event_type == Disk.EVENT_FAIL:
            disk = self.disks[diskId]
    ...
    def manual_inject_failures(self, fail_report, simulate):
        ...
        if self.manual_spool_fail:
            ...
            manual_spool.failed_disks_network_repair.clear()
        ...
        for item in fail_report['detect_queue']:
            ...
            # if e_type == Disk.EVENT_DETECT:
            #     print('yes!')
        mlec_d_sodp_repair(self, self.simulation.repair_queue)
